...
local Script_Title = "vl自瞄 <font color=\"rgb(255, 0, 0)\">|</font> 锁头/身体 <font color=\"rgb(255, 0, 0)\">|</font> vl自瞄"
...
local toggleBtn
toggleBtn = addToggle("打开/关闭锁定", function(state)
	enabled = state
end, false)

local toggleTeamCheckBtn
toggleTeamCheckBtn = addToggle("团队检测", function(state)
	teamcheck = state
end)

local aimPartCombo
aimPartCombo = addComboBox("选择", {"头", "躯干"}, function(selection)
	aimpart = selection
end)
